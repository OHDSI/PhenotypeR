...
knee_oa <- tibble(name = "knee_osteoarthritis",
                  estimate = c("Median age", "Proportion male"),
                  value = c("60 to 65", "45%"))
knee_replacement <- tibble(name = "knee_replacement",
                           estimate = c("Median age", "Proportion male"),
                           value = c("65 to 70", "50%"))
```
...
Here for example we'll use Google Gemini to populate our expectations. 
...
getCohortExpectations(chat = chat, 
                      phenotpyes = c("ankle sprain", "prostate cancer", "morphine")) |> 
  tableCohortExpectations()
```

Instead of passing our cohort names, we could instead pass our results set from `phenotypeDiagnostics()` instead. In this case we'll automaitcally get expectations for each of the study cohorts in our results.
```{r, eval=FALSE}
library(DBI)
library(duckdb)
